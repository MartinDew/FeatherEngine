...
-- The public API surface that gets bindings: core's headers.
-- Modules are deliberately out -- they're implementation, reachable through core's interfaces.
...
rule("feather.mrbind_api")
    on_config(function (target)
        ...
        -- A changed header list means a header was added or removed, which an mtime comparison alone can miss
        -- (removing one leaves every remaining file untouched).
...
-- Phony: produces files, not a linkable artifact. Everything generating bindings depends on it, putting its parse ahead of
-- their generation steps.
target("bindings_api")
    set_kind("phony")
    set_group("bindings")
    ...
    add_packages("mrbind")
    -- Windows-only: mrbind is built against libllvm's clang there rather than any system clang (thirdparty/xmake.lua), and resolving
    -- that clang needs a direct target:pkg("libllvm") handle.
    if is_plat("windows") then
        add_packages("libllvm")
    end

    -- Same defines the engine's own targets set (xmake/engine.lua, feather_module_target); without them the parse sees a different
    -- API than the one that gets compiled, since #if BETA members would disappear.
