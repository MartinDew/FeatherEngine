...
generate_register_cpp; a modifier overrides only the hooks it needs, the rest
default to "contribute nothing".

Modifiers are scoped per source-directory pass (see build_registry): built-in
ones always load, --extension ones only for the directory they're scoped to.
This is load-bearing -- core/ is regenerated by every pass, so a differing
extension set between passes would make core's output depend on who ran the
tool, defeating write_if_changed's whole point.
"""
