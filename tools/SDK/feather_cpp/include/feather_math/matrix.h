...
// FEATHER_MATH_STANDALONE compiles these types with no reflection at all,
// which is how a plugin gets them: identical fields and layout, without the
// ClassDB machinery an FSTRUCT would otherwise drag in.
#ifdef FEATHER_MATH_STANDALONE
#define FSTRUCT(...)
#else
...
struct Matrix4x4f {
	...
	// ---- Projections -------------------------------------------------------
	// Right-handed, looking down -Z, mapping depth to [0, 1] -- what the
	// renderer expects, and what RTM's own camera helpers produce.

	static Matrix4x4f perspective(float fov_y_radians, float aspect_ratio, float near_plane, float far_plane) {
		return from_rtm(rtm::proj_perspective_fov(fov_y_radians, aspect_ratio, near_plane, far_plane));
	}
	static Matrix4x4f orthographic(float width, float height, float near_plane, float far_plane) {
		return from_rtm(rtm::proj_orthographic(width, height, near_plane, far_plane));
	}

	// The off-center forms, which RTM does not provide: an asymmetric frustum,
	// used for a viewport that is not centred on the view axis.
	static Matrix4x4f
	perspective_off_center(float left, float right, float bottom, float top, float near_plane, float far_plane);
	static Matrix4x4f
	orthographic_off_center(float left, float right, float bottom, float top, float near_plane, float far_plane);

	// Looks from `position` toward `target`, i.e. the inverse of that camera's
	// transform.
	static Matrix4x4f look_at(const Vector3f& position, const Vector3f& target, const Vector3f& up) {
		const rtm::matrix3x4f m = rtm::view_look_at(position.to_rtm(), target.to_rtm(), up.to_rtm());
		return from_rtm(rtm::matrix_cast(m));
	}
	...
	// Splits the matrix into the translation, rotation and scale that produced
	// it. False when an axis has no length, which no rotation can be read from.
	bool decompose(Vector3f& out_translation, Quaternionf& out_rotation, Vector3f& out_scale) const;
