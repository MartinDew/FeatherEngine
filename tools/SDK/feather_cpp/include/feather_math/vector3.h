...
struct Vector3f {
	...
	// Component-wise 1/x, with components at or below `tolerance` left at zero
	// rather than becoming infinite -- what inverting a scale needs.
	[[nodiscard]] Vector3f reciprocal_safe(float tolerance = 1.e-4f) const {
		return { std::abs(x) <= tolerance ? 0.0f : 1.0f / x,
				 std::abs(y) <= tolerance ? 0.0f : 1.0f / y,
				 std::abs(z) <= tolerance ? 0.0f : 1.0f / z };
	}
	[[nodiscard]] bool any_component_less_equal(float value) const { return x <= value || y <= value || z <= value; }
