...
`xmake export-api` publishes `build/bindings/dist/feather_api.json` — the API description external plugin projects build against (see Plugin system below). It rewrites the engine checkout's own path into the `@feather` token the SDK substitutes back, so the file names no machine. It must produce *only* that one JSON file; nothing binary.
...
`tools/SDK/` is meant to be **vendored into an external plugin repo**, not consumed from this checkout. `FeatherPluginSDK.lua`, `modules/feather_plugin_bindings.lua` and `packages/` are shared; each language then has its own directory holding what only it needs — `feather_c/`, `feather_cpp/` (the `gen_cpp/` generator and the hand-written `include/feather_cpp/` headers, alongside `include/feather_math/`, which the build refreshes from `core/math` so the engine and its plugins compile one copy) and `feather_cs/` (`FeatherPluginBootstrap.cs`). A plugin repo vendors the shared files plus the languages it builds; each directory defines that language's `feather_<lang>_plugin()`. Combined with a published `feather_api.json` (from `xmake export-api`), it's the whole toolchain a plugin author needs — no engine checkout, no Clang/LLVM (`mrbind_generators.lua` builds mrbind's generators only, `MRBIND_BUILD_PARSER=OFF`). `tools/templates/plugin_c_template/`, `plugin_cs_template/` and `plugin_cpp_template/` are starting points.

The math types are the one thing a C++ plugin does not reach through a wrapper: it compiles the same `core/math` sources the engine did (vendored into `tools/SDK/feather_cpp/include/feather_math/` by `feather_bindings.sync_math_headers`, with `FEATHER_MATH_STANDALONE` compiling the reflection out), so `Vector2/3/4`, `Quaternion` and `Color` cross the C boundary as real structs and `Matrix` as a pointer to one. The generated `feather_cpp/math.hpp` asserts every size and field offset the engine published, so a layout disagreement is a compile error rather than reinterpreted memory. A C# plugin needs no hand-written entry point or P/Invoke resolver — `FeatherPluginBootstrap.cs` supplies both and finds `[FeatherComponent]`/`[FeatherSystem]`/`[FeatherInit]`-attributed types by reflecting over the assembly at a fixed, SDK-owned entry point.
