...
`tools/SDK/` is meant to be **vendored into an external plugin repo**, not consumed from this checkout. `FeatherPluginSDK.lua` (`feather_plugin_sdk_init()` — moduledirs, package requires, and `includes()` of the rules), `feather_plugin_rules.lua` and `modules/feather_plugin_bindings.lua` and `packages/` are shared; C needs nothing language-specific, `feather_cpp/` holds the `gen_cpp/` generator, the hand-written `include/feather_cpp/` headers, DirectXMath's package and the `thirdparty/` SimpleMath/DirectXMath sources both sides compile, and `feather_cs/` holds `FeatherPluginBootstrap.cs` plus `Directory.Build.targets`. A plugin declares a plain `target()` and adds `feather.plugin.c` / `.cpp` / `.cs` (all defined in `feather_plugin_rules.lua`, layered on `feather.plugin.common`); `set_values("feather.api_json", …)` points at the API file, defaulting to `api/feather_api.json`. The C# rule sits on xmake's built-in `csharp` rule — `set_values("csharp.*", …)` becomes the `.csproj`, and a target-level build script (installed from the rule's `on_load`, which suppresses every rule's build script) publishes that project with NativeAOT instead of the stock `dotnet build`; `Directory.Build.targets` carries the two ItemGroups xmake's csproj generator can't emit (the generated-source glob, the trimmer root). Combined with a published `feather_api.json` (from `xmake export-api`), it's the whole toolchain a plugin author needs — no engine checkout, no Clang/LLVM (`mrbind_generators.lua` builds mrbind's generators only, `MRBIND_BUILD_PARSER=OFF`). `tools/templates/plugin_c_template/`, `plugin_cs_template/` and `plugin_cpp_template/` are starting points.
